...
**特点**：
- 🔍 全面识别 - 市内交通、长途交通、住宿、退票费、餐饮等五大分类
- 🧮 Python 金额校验 - 强制用代码验算，防止 LLM 心算错误
- 🚄 铁路客票归长途 - 所有铁路电子客票不论距离一律归长途交通
- 🎫 退票费独立分类 - 火车/机票退票费单独统计
- 📄 报告文件名优化 - 使用文件夹名称命名
- 📊 结构化摘要卡片 - 处理完成后输出可视化摘要
- 📝 报销汇总追加 - 可通过 config.json 追加到汇总 MD 顶部
- 🍽️ 招待费/客户模式 - 输入含招待费、招待或客户时逐张输出招待费明细
- 🔄 智能去重 - 按发票号码、行程单/结账单关系去重

[查看详情 →](./wlzh-invoice-scanner/)
...
### 🐦 x-twitter-scraper
通过 Xquik API 为 Agent 提供 X (Twitter) 数据工作流，适合需要结构化搜索、用户资料、粉丝导出、媒体下载、MCP、Webhook 或需确认后发布的场景。

**特点**：
- 🔎 推文搜索与互动数据查询
- 👤 用户资料、时间线、媒体推文、粉丝和关注列表
- 📦 批量抽取任务，执行前先估算并确认
- 🔌 MCP、SDK、Webhook 和监控工作流
- 🔐 仅使用 `XQUIK_API_KEY`，不收集 X 密码、2FA、Cookie 或会话
- ✅ 私密读取、写操作、监控、Webhook 和计费批量任务都需要用户明确确认
