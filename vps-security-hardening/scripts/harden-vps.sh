...
echo ""
echo -e "  ${CYAN}# 复制下面整段命令，粘贴执行即可${NC}"
echo ""
echo -e "  ${GREEN}sed -i 's/^#\\?PasswordAuthentication.*/PasswordAuthentication yes/' /etc/ssh/sshd_config && \\${NC}"
echo -e "  ${GREEN}sed -i 's/^#\\?PermitRootLogin.*/PermitRootLogin yes/' /etc/ssh/sshd_config && \\${NC}"
echo -e "  ${GREEN}sed -i 's/^#\\?PasswordAuthentication.*/PasswordAuthentication yes/' /etc/ssh/sshd_config.d/*.conf 2>/dev/null; \\${NC}"
echo -e "  ${GREEN}sed -i 's/^#\\?PermitRootLogin.*/PermitRootLogin yes/' /etc/ssh/sshd_config.d/*.conf 2>/dev/null; \\${NC}"
echo -e "  ${GREEN}systemctl restart ssh && \\${NC}"
echo -e "  ${GREEN}passwd root${NC}"
echo ""
echo -e "${YELLOW}⚠️  执行 passwd root 后，输入密码: 123${NC}"
echo -e "${YELLOW}⚠️  如果之前使用密钥登录，请确保记住密码后再继续${NC}"
echo ""
...
$SSH_CMD "
    # Step 1: Comment out ALL conflicting directives in main sshd_config
    # This ensures only sshd_config.d/99-hardening.conf takes effect
    # 
    # We need to handle both:
    #   - Directives starting with the keyword (e.g., 'Port 22')
    #   - Commented directives (e.g., '#Port 22')
    ...
    echo 'Commented out all conflicting directives in /etc/ssh/sshd_config'

    # Step 2: Remove all existing configs in sshd_config.d directory
    # Backup and remove to avoid any conflicts
    ...
    # Step 3: Create our hardening config (the only active config in sshd_config.d)
...
# Date: $(date +%Y-%m-%d)
# 
# NOTE: This is the ONLY active SSH config file.
# All conflicting directives in /etc/ssh/sshd_config have been commented out.
# All other configs in /etc/ssh/sshd_config.d/ have been backed up and disabled.
