...
echo ""
echo -e "  ${CYAN}# 2. 检查并修改 sshd_config.d 目录下的配置（Ubuntu 22.04+）${NC}"
echo -e "  ${CYAN}for f in /etc/ssh/sshd_config.d/*.conf; do${NC}"
echo -e "  ${CYAN}  [ -f \"\$f\" ] && sed -i 's/^#\\?PasswordAuthentication.*/PasswordAuthentication yes/' \"\$f\"${NC}"
echo -e "  ${CYAN}  [ -f \"\$f\" ] && sed -i 's/^#\\?PermitRootLogin.*/PermitRootLogin yes/' \"\$f\"${NC}"
echo -e "  ${CYAN}done${NC}"
echo ""
echo -e "  ${CYAN}# 3. 重启 SSH 服务并设置 root 密码${NC}"
echo -e "  ${CYAN}systemctl restart ssh${NC}"
echo -e "  ${CYAN}passwd root${NC}"
echo ""
echo -e "${YELLOW}⚠️  注意: 执行完上述命令后，root 密码登录将开启${NC}"
echo -e "${YELLOW}⚠️  注意: 如果之前使用密钥登录，请确保记住密码后再继续${NC}"
echo ""
